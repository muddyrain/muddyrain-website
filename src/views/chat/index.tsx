...
export default function Page() {
  ...
  const [currentActiveUser, setCurrentActiveUser] = useChatStore(state => [
    state.currentActiveUser,
    state.setCurrentActiveUser,
  ])
  ...
  return (
    ...
    >
              ...
              {userList.map((item, index) => (
                <div
                  className={`h-20 flex items-center pr-4 px-3 pl-3 border-0 border-b border-solid border-zinc-200 last:border-b-0 cursor-pointer duration-300 ${
                    currentActiveUser?.id === item.id ? 'bg-zinc-100 ' : 'bg-white'
                  }`}
                  onClick={() => {
                    if (currentActiveUser?.id === item.id) {
                      setCurrentActiveUser(null)
                    } else {
                      setCurrentActiveUser(item)
                    }
                  }}
                  key={index}
                >
                  <Avatar className="mr-2" src={item.avatar} />
      ...
      {currentActiveUser ? (
        ...
        <div className="flex-1 flex flex-col ml-4 bg-white flex justify-center items-center">
          <Empty />
        </div>
      )}
    </div>
